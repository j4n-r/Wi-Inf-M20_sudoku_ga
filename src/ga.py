...
def batch_tournament_winners(
    fitness_scores: np.ndarray, selection_count: int, tournament_members: int = 3
) -> np.ndarray:
    """
    ...
    candidates = my_rng.integers(
        0, population_size, size=(selection_count, tournament_members)
    )
    ...
    winners = np.take_along_axis(candidates, winner_offsets[:, None], axis=1).reshape(
        -1
    )
    return winners
...
def evolve_population(
    ...
    parent_indices_1 = batch_tournament_winners(
        fitness_scores, offspring_count, tournament_members
    )
    parent_indices_2 = batch_tournament_winners(
        fitness_scores, offspring_count, tournament_members
    )
    crossover_points = my_rng.integers(1, GRID_SIZE, size=offspring_count)

    # iterate over all three arrays at the same time
    for offset, (p1_idx, p2_idx, point) in enumerate(
        zip(parent_indices_1, parent_indices_2, crossover_points)
    ):
