...
ROW_MUTABLE = [[j for j in range(9) if SUDOKU[i][j] == 0] for i in range(9)]
...
def benchmark(pop_size: int, parallel: bool) -> tuple[float, int]:
    ...
    while True:
        if parallel:
            fitness = calculate_fitness_parallel(population, worker_pool, CHUNK_SIZE)
        else:
            fitness = calculate_fitness_population(population)
